...
{
  if (argc != 3) {
    std::cerr << argv[0] << " read-chunk-size(KB) num_drives" << std::endl;
    exit(1);
  }
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    if (pthread_create(&tid[i], NULL, io_seq_reader, (void *) &arg[i]) != 0) {
  ...
  std::cout << "access_length: " << read_chunk_size << " threads: " << num_drives << " gps: " << gps_total << std::endl;
...
void *io_seq_reader(void *p)
{
  ...
  for (u64 i = 0; i < NUM_REQUESTS; ++i) {
    if (!_pread(arg->fd, bp, read_chunk_size, offset)) {
      perror("pread");
    }
